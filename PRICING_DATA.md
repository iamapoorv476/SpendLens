# PRICING_DATA.md

Every number in the audit engine traces to an official vendor pricing page.
Format: Plan: $X/user/month — URL — verified YYYY-MM-DD

---

## Cursor

- Hobby: $0/month — https://cursor.com/pricing — verified 2026-05-09
- Pro: $20/user/month — https://cursor.com/pricing — verified 2026-05-09
- Pro+: $60/user/month — https://cursor.com/pricing — verified 2026-05-09
- Ultra: $200/user/month — https://cursor.com/pricing — verified 2026-05-09
- Teams: $40/user/month — https://cursor.com/pricing — verified 2026-05-09
- Enterprise: custom pricing, contact sales — https://cursor.com/pricing — verified 2026-05-09

Note: Since June 2025, Cursor uses credit-based billing. Each paid plan includes a monthly credit pool equal to the plan price (e.g., Pro includes $20 of model credits). Auto mode is unlimited and does not consume credits. Overages billed at API rates. Source: https://cursor.com/pricing — verified 2026-05-09

---

## GitHub Copilot

- Free: $0/month — https://github.com/features/copilot/plans — verified 2026-05-09
- Pro: $10/user/month — https://github.com/features/copilot/plans — verified 2026-05-09
- Pro+: $39/user/month — https://github.com/features/copilot/plans — verified 2026-05-09
- Business: $19/user/month — https://github.com/features/copilot/plans — verified 2026-05-09
- Enterprise: $39/user/month — https://github.com/features/copilot/plans — verified 2026-05-09

Note: GitHub Copilot transitions to usage-based billing (GitHub AI Credits) on June 1, 2026. Base plan prices are unchanged. Code completions remain unlimited and do not consume credits. Source: https://github.blog/news-insights/company-news/github-copilot-is-moving-to-usage-based-billing/ — verified 2026-05-09

---

## Claude (Anthropic) — Subscription

- Free: $0/month — https://anthropic.com/pricing — verified 2026-05-09
- Pro: $20/user/month ($17/month billed annually) — https://anthropic.com/pricing — verified 2026-05-09
- Max: $100/user/month — https://anthropic.com/pricing — verified 2026-05-09
- Team: custom per seat, minimum 5 members — https://anthropic.com/pricing — verified 2026-05-09
- Enterprise: custom pricing, contact sales — https://anthropic.com/pricing — verified 2026-05-09

## Anthropic API (Claude) — per million tokens

- Claude Haiku 4.5: $1.00 input / $5.00 output — https://anthropic.com/pricing — verified 2026-05-09
- Claude Sonnet 4.6: $3.00 input / $15.00 output — https://anthropic.com/pricing — verified 2026-05-09
- Claude Opus 4.7: $5.00 input / $25.00 output — https://anthropic.com/pricing — verified 2026-05-09

Note: Batch API gives 50% discount across all models. Prompt caching reduces cached input cost by up to 90%. Source: https://anthropic.com/pricing — verified 2026-05-09

---

## ChatGPT (OpenAI) — Subscription

- Free: $0/month — https://chatgpt.com/pricing/ — verified 2026-05-09
- Plus: $20/user/month — https://chatgpt.com/pricing/— verified 2026-05-09
- Pro: $200/user/month - https://chatgpt.com/pricing/ — verified 2026-05-09
- Business ChatGPT & Codex: ₹1,800/user/month in India region pricing (pricing varies by region) — https://chatgpt.com/pricing/ — verified 2026-05-09
- Enterprise: custom pricing, contact sales — https://chatgpt.com/pricing/ — verified 2026-05-09

## OpenAI API — per million tokens

- GPT-5.4: $2.50 input / $15.00 output — https://openai.com/api/pricing/ — verified 2026-05-09
- GPT-5.5: $5.00 input / $30.00 output — https://openai.com/api/pricing/ — verified 2026-05-09

Note: Batch API gives 50% discount. Cached input gives additional reduction. Source: https://openai.com/api/pricing/ — verified 2026-05-09

---

## Google Gemini — Subscription

- Free: $0/month — https://one.google.com/about/ai-premium — verified 2026-05-09
- Google AI Plus: $7.99/month — https://one.google.com/about/ai-premium — verified 2026-05-09
- Google AI Pro: $19.99/month — https://one.google.com/about/ai-premium — verified 2026-05-09
- Google AI Ultra: $249.99/month — https://one.google.com/about/ai-premium — verified 2026-05-09

## Gemini API — per million tokens

- Gemini 3.1 Flash-Lite: $0.10 input / $0.40 output — https://ai.google.dev/gemini-api/docs/pricing — verified 2026-05-09
- Gemini 3 Flash: $0.50 input / $3.00 output — https://ai.google.dev/gemini-api/docs/pricing — verified 2026-05-09
- Gemini 2.5 Pro: $1.25 input / $10.00 output (up to 200K context) — https://ai.google.dev/gemini-api/docs/pricing — verified 2026-05-09
- Gemini 3.1 Pro: $2.00 input / $12.00 output (up to 200K context) — https://ai.google.dev/gemini-api/docs/pricing — verified 2026-05-09

Note: Prompts exceeding 200K tokens charged at ~2x rate. Batch API gives 50% discount. Source: https://ai.google.dev/gemini-api/docs/pricing — verified 2026-05-09

---

## Windsurf

- Free: $0/month — https://windsurf.com/pricing — verified 2026-05-09
- Pro: $20/user/month — https://windsurf.com/pricing — verified 2026-05-09
- Max- $200/month -https://windsurf.com/pricing — verified 2026-05-09
- Teams: $40/user/month — https://windsurf.com/pricing — verified 2026-05-09
- Enterprise: custom pricing, contact sales — https://windsurf.com/pricing — verified 2026-05-09